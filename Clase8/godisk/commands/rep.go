...
func ExecuteRep(name, path, id string) {
	...
	case "mbr":
		...
		// CORREGIDO: Pasar los argumentos que la función espera (file y un puntero a mbr)
		reportContent = reports.GenerateMbrReport(file, &mbr)
	...
	case "inode":
		...
		reportContent = reports.GenerateInodeReport(file, sb)
