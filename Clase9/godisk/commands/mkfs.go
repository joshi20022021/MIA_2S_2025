...
func ExecuteMkfs(id, formatType, fsType string) {
	...
	// Calcular posición del journal después de los bloques
	superbloque.S_journal_start = superbloque.S_block_start + (superbloque.S_blocks_count * superbloque.S_block_size)
	superbloque.S_journal_count = 0
	...
	// --- 12. INICIALIZACIÓN DEL JOURNAL ---
	// Crear un journal vacío con entradas inicializadas en cero
	emptyEntry := structs.JournalEntry{}

	// Escribir 1000 entradas vacías en el journal
	file.Seek(int64(superbloque.S_journal_start), 0)
	for i := 0; i < 1000; i++ {
		if err := binary.Write(file, binary.BigEndian, &emptyEntry); err != nil {
			fmt.Println("Error al inicializar el journal:", err)
			return
		}
	}
	fmt.Println("Journal inicializado correctamente.")
	...
	fmt.Println("Sistema de archivos creado exitosamente en la partición, incluyendo users.txt y journal.")
}
