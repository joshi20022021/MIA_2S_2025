...
func executeCommand(commandLine string) string {
	...
	case "cat":
		...
		file1 := catCmd.String("file1", "", "Primer archivo a mostrar.")
		file2 := catCmd.String("file2", "", "Segundo archivo a mostrar.")
		file3 := catCmd.String("file3", "", "Tercer archivo a mostrar.")
		file4 := catCmd.String("file4", "", "Cuarto archivo a mostrar.")
		file5 := catCmd.String("file5", "", "Quinto archivo a mostrar.")
		_ = catCmd.Parse(args)

		// Recopilar todos los archivos especificados
		...
		if *file3 != "" {
			filePaths = append(filePaths, *file3)
		}
		if *file4 != "" {
			filePaths = append(filePaths, *file4)
		}
		if *file5 != "" {
			filePaths = append(filePaths, *file5)
		}

		commands.ExecuteCat(filePaths)
	...
	case "mkdir":
		...
		pFlag := mkdirCmd.Bool("p", false, "Crear directorios padres si no existen.")
		_ = mkdirCmd.Parse(args)
		commands.ExecuteMkdir(*path, *pFlag)
