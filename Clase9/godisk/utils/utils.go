...
// comandos nuevos para MKFILE
// WriteObject escribe cualquier struct en una posición específica de un archivo binario.
func WriteObject(file *os.File, position int64, data interface{}) error {
	// Mover el cursor a la posición deseada
	_, err := file.Seek(position, 0)
	if err != nil {
		return err
	}
	// Escribir los datos en formato BigEndian
	return binary.Write(file, binary.BigEndian, data)
}

// SetBit escribe un solo byte ('0' o '1') en una posición del bitmap.
func SetBit(file *os.File, startByte int64, index int32, value byte) error {
	// Mover el cursor a la posición del bit
	_, err := file.Seek(startByte+int64(index), 0)
	if err != nil {
		return err
	}
	// Escribir el byte
...
// FindFreeBit busca el primer bit libre ('0') en un bitmap y devuelve su índice.
func FindFreeBit(file *os.File, startByte int64, count int32) (int32, error) {
	buffer := make([]byte, 1)
	for i := int32(0); i < count; i++ {
		// Mover el cursor a la posición del bit a leer
		_, err := file.Seek(startByte+int64(i), 0)
		if err != nil {
			return -1, err
		}
		// Leer el bit
		...
		// Si es '0', hemos encontrado un espacio libre
		if buffer[0] == '0' {
			return i, nil
		}
	}
	// Si el bucle termina, no hay espacio
	return -1, errors.New("no se encontró espacio libre en el bitmap")
}

// AddJournalEntry agrega una entrada al journal del sistema de archivos
func AddJournalEntry(file *os.File, sb *structs.Superblock, operation, path, content, date string) error {
	if sb.S_journal_count >= 1000 { // Límite de entradas del journal
		return errors.New("journal lleno")
	}

	// Crear la entrada del journal
	var entry structs.JournalEntry

	// Copiar los strings a los arrays de bytes
	copy(entry.Operation[:], operation)
	copy(entry.Path[:], path)
	copy(entry.Content[:], content)
	copy(entry.Date[:], date)

	// Calcular la posición donde escribir la entrada
	entrySize := int64(binary.Size(entry))
	journalEntryPos := int64(sb.S_journal_start) + (int64(sb.S_journal_count) * entrySize)

	// Escribir la entrada
	_, err := file.Seek(journalEntryPos, 0)
	if err != nil {
		return err
	}
...
// ReadJournal lee todas las entradas del journal
func ReadJournal(file *os.File, sb *structs.Superblock) ([]structs.JournalEntry, error) {
	var entries []structs.JournalEntry

	if sb.S_journal_count == 0 {
		return entries, nil
	}
	...
	for i := int32(0); i < sb.S_journal_count; i++ {
		journalEntryPos := int64(sb.S_journal_start) + (int64(i) * entrySize)
